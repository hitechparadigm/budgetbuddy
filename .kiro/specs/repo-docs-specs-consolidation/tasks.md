...
- [ ] 1. Draft `.kiro/specs/README.md` spec index content
...
- [ ] 2. Draft rewritten `docs/README.md` doc index content
...
- [ ] 3. Draft steering rule additions as new sections
...
- [ ] 4. Draft `.config.kiro` content for each of the 19 specs
...
- [ ] 5. Checkpoint — review groundwork drafts with user
...
- [ ] 6. Move `competitive-features` out of archive and set metadata
...
- [ ] 7. Move `critical-bug-fixes` out of archive and set metadata
...
- [ ] 8. Move `documentation-cleanup` out of archive and set metadata
...
- [ ] 9. Move `documentation-validation-fix` out of archive and set metadata
...
- [ ] 10. Move `enhanced-accounts-transactions` out of archive and set metadata
...
- [ ] 11. Move `hooks-optimization` out of archive and set metadata
  - Move `.kiro/specs/archive/hooks-optimization/` to `.kiro/specs/hooks-optimization/`
  - Add/update `.config.kiro` with `status: "superseded"`, `category: "process"`
  - _Requirements: 1.1, 2.1, 2.2, 2.3, 3.3_

- [ ] 12. Move `mobile-ui-polish` out of archive and set metadata
  - Move `.kiro/specs/archive/mobile-ui-polish/` to `.kiro/specs/mobile-ui-polish/`
  - Add/update `.config.kiro` with `status: "complete"`, `category: "feature"`
  - _Requirements: 1.1, 2.1, 2.2, 2.3, 3.2_

- [ ] 13. Move `multi-currency` out of archive and set metadata
  - Move `.kiro/specs/archive/multi-currency/` to `.kiro/specs/multi-currency/`
  - Add/update `.config.kiro` with `status: "complete"`, `category: "feature"`
  - _Requirements: 1.1, 2.1, 2.2, 2.3, 3.2_

- [ ] 14. Move `onboarding-403-fix` out of archive and update existing metadata
...
- [ ] 15. Move `plan-model-redesign` out of archive and set metadata
...
- [ ] 16. Move `test-coverage-improvement` out of archive and set metadata
  - Move `.kiro/specs/archive/test-coverage-improvement/` to `.kiro/specs/test-coverage-improvement/`
  - Add/update `.config.kiro` with `status: "complete"`, `category: "process"`
  - _Requirements: 1.1, 2.1, 2.2, 2.3, 3.3_

- [ ] 17. Move `ui-polish-enhancements` out of archive and set metadata
  - Move `.kiro/specs/archive/ui-polish-enhancements/` to `.kiro/specs/ui-polish-enhancements/`
  - Add/update `.config.kiro` with `status: "complete"`, `category: "feature"`
  - _Requirements: 1.1, 2.1, 2.2, 2.3, 3.2_

- [ ] 18. Set `.config.kiro` metadata on the 7 previously-active specs
...
- [ ] 19. Delete the now-empty `.kiro/specs/archive/` directory
  - Confirm `.kiro/specs/archive/` contains zero remaining subdirectories after tasks 6–17, then delete it
  - _Requirements: 1.2, 1.3_

- [ ] 20. Commit the finalized `.kiro/specs/README.md` index
...
- [ ] 22. Merge `stack-management-guide.md`'s unique content into `docs/aws-stack-architecture.md`
...
- [ ] 23. Merge `aws-resource-standards.md`'s content into `docs/aws-stack-architecture.md`
...
- [ ] 24. Merge `user-guide-notifications.md` into `docs/user-guide-budget-collaboration.md`
...
- [ ] 25. Merge `multi-currency-guide.md` into `docs/user-guide-budget-collaboration.md`
...
- [ ] 26. Correct `docs/localstack-guide.md`'s stale Lambda reference
...
- [ ] 28. Delete the three merged architecture source files and the stale diagram
...
- [ ] 29. Delete the three merged user-guide source files
...
- [ ] 30. Delete obsolete top-level `/docs` files
...
- [ ] 31. Delete `docs/archive/` top-level one-off summary files
...
- [ ] 32. Delete `docs/archive/blockers/` contents and the directory
...
- [ ] 33. Delete `docs/archive/sessions/` contents and the directory
...
- [ ] 34. Delete `docs/archive/kiro/` contents and the directory
...
- [ ] 37. Remove deleted-doc references from `.kiro/README.md` and `.kiro/SYSTEM_GUIDE.md`
...
- [ ] 38. Correct stale "in progress" framing in package-level READMEs
...
- [ ] 40. Delete stray root-level image files
...
- [ ] 41. Delete the untracked junk directory at the repository root
  - Delete the directory with the literal name `` test-I`U&M9X `` at the repository root
  - _Requirements: 8.2_

- [ ] 42. Verify Group E completion
...
- [ ] 43. Append spec lifecycle, doc placement, and no-stray-artifact rules to `structure.md`
...
- [ ] 44. Append doc index maintenance rule to `documentation-standards.md`
...
- [ ] 46. Full consolidation completion verification
