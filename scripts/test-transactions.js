...
async function authenticateUser() {
    try {
        ...
        if (error.code === 'UserNotFoundException') {
            console.log('💡 You need to create a test user first. Run: npm run create-test-user');
        } else if (error.code === 'NotAuthorizedException') {
            console.log('💡 Check your username/password or user might need confirmation');
        }
...
async function testTransactionCRUD() {
    ...
    try {
        ...
        const getResponse = await makeRequest('GET', '/transactions', null, accessToken);
        console.log(`Status: ${getResponse.status}`);
        if (getResponse.status === 200) {
            console.log(`✅ Retrieved ${getResponse.data.data.transactions.length} transactions`);
        } else {
        ...
        for (let i = 0; i < testTransactions.length; i++) {
            ...
            console.log(`   Creating ${transaction.type}: $${transaction.amount} - ${transaction.description}`);

            const createResponse = await makeRequest('POST', '/transactions', transaction, accessToken);
            console.log(`   Status: ${createResponse.status}`);

            if (createResponse.status === 200 || createResponse.status === 201) {
                createdTransactions.push(createResponse.data.data.transaction);
                console.log(`   ✅ Created transaction ID: ${createResponse.data.data.transaction.transactionId}`);
            } else {
        ...
        const getResponse2 = await makeRequest('GET', '/transactions', null, accessToken);
        if (getResponse2.status === 200) {
            console.log(`✅ Retrieved ${getResponse2.data.data.transactions.length} transactions`);
        ...
        if (createdTransactions.length > 0) {
            ...
            const getSingleResponse = await makeRequest('GET', `/transactions/${transactionId}`, null, accessToken);

            if (getSingleResponse.status === 200) {
                console.log(`✅ Retrieved transaction: ${getSingleResponse.data.data.transaction.description}`);
            } else {
                console.log('❌ Failed to get single transaction:', getSingleResponse.data);
            }
        }
        ...
        if (createdTransactions.length > 0) {
            ...
            const updateResponse = await makeRequest('PUT', `/transactions/${transactionId}`, updateData, accessToken);

            if (updateResponse.status === 200) {
                console.log(`✅ Updated transaction: $${updateResponse.data.data.transaction.amount}`);
            } else {
        ...
        // Filter by type
        const expenseResponse = await makeRequest('GET', '/transactions?type=expense', null, accessToken);
        if (expenseResponse.status === 200) {
            console.log(`✅ Expense filter: ${expenseResponse.data.data.transactions.length} expenses found`);
        }

        const incomeResponse = await makeRequest('GET', '/transactions?type=income', null, accessToken);
        if (incomeResponse.status === 200) {
            console.log(`✅ Income filter: ${incomeResponse.data.data.transactions.length} income found`);
        }
